...
var margem_enderecos_titulo: float
var margem_entre_enderecos = 5
var margem_entre_endereco_canto = 40
...
func _ready() -> void:
	...
	adicionar_original["margem"] = Vector2(720, 1280) - $adicionar.global_position - $adicionar.size
	nenhum_original_font_size = $nenhum.get_theme_font_size("font_size")
	margem_enderecos_titulo = $enderecos.global_position.y - $enderecos_texto.global_position.y - $enderecos_texto.size.y
	carregar_enderecos()
...
func atualizar_interface() -> void:
	if get_viewport_rect().size.y / get_viewport_rect().size.x >= 1:
		...
		$adicionar.global_position.x = get_viewport_rect().size.x - $adicionar.size.x - adicionar_original["margem"].x * get_viewport_rect().size.x / 720
		$adicionar.global_position.y = get_viewport_rect().size.y - $adicionar.size.y - adicionar_original["margem"].y * get_viewport_rect().size.y / 1280
		...
		$enderecos.global_position.y = $enderecos_texto.global_position.y + $enderecos_texto.size.y + margem_enderecos_titulo * get_viewport_rect().size.y / 1280
		for index in range(0, len($enderecos.get_children()), 1):
			$enderecos.get_child(index).atualizar_interface(margem_entre_endereco_canto * get_viewport_rect().size.x / 720,
															margem_entre_enderecos * get_viewport_rect().size.x / 720 + (10 * get_viewport_rect().size.x / 720 + $enderecos.get_child(index).size.y) * index)
		...
		$adicionar.size.x = adicionar_original["fundo_size"].x * get_viewport_rect().size.x / 720
		$adicionar.size.y = adicionar_original["fundo_size"].y * get_viewport_rect().size.x / 720
		$adicionar.global_position.x = get_viewport_rect().size.x - $adicionar.size.x - adicionar_original["margem"].x * get_viewport_rect().size.x / 1280
		$adicionar.global_position.y = get_viewport_rect().size.y - $adicionar.size.y - adicionar_original["margem"].y * get_viewport_rect().size.y / 720
		$nenhum.add_theme_font_size_override("font_size", nenhum_original_font_size * get_viewport_rect().size.x / 720)
		$nenhum.reset_size()
		$nenhum.position.x = (get_viewport_rect().size.x - $nenhum.size.x) / 2
		$nenhum.position.y = (get_viewport_rect().size.y - $nenhum.size.y) / 2
		$enderecos.global_position.y = $enderecos_texto.global_position.y + $enderecos_texto.size.y + margem_enderecos_titulo * get_viewport_rect().size.y / 720
		for index in range(0, len($enderecos.get_children()), 1):
			$enderecos.get_child(index).atualizar_interface(margem_entre_endereco_canto * get_viewport_rect().size.x / 1280 + ($enderecos.get_child(index).size.x + 40 * get_viewport_rect().size.x / 1280) * (index % 2),
															margem_entre_enderecos * get_viewport_rect().size.x / 1280 + (10 * get_viewport_rect().size.x / 1280 + $enderecos.get_child(index).size.y) * floori(index / 2.0))
		if $enderecos.get_child_count() > 0:
			size.y = $enderecos.get_child(-1).global_position.y + $enderecos.get_child(-1).size.y
		elif $enderecos.get_child_count() == 0:
			size.y = $enderecos_texto.global_position.y + $enderecos_texto.size.y
